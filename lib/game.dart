...
class _FludoGameState extends State<FludoGame> with TickerProviderStateMixin {
  ...
  late CollisionDetails _collisionDetails = CollisionDetails();

  int _stepCounter = 0, _diceOutput = 0, _currentTurn = 0, _selectedPawnIndex = 0, _maxTrackIndex = 57, _straightSixesCounter = 0, _forwardStepAnimTimeInMillis = 250, _reverseStepAnimTimeInMillis = 60;
  late List<List<List<Rect>>> _playerTracks;
  late List<Rect> _safeSpots;
  late List<List<MapEntry<int, Rect>>> _pawnCurrentStepInfo = []; //step index, rect
  ...
  void initState() {
    ...
    print("this is abra klollol" + player.toString());
    Future.delayed(Duration.zero, () {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return BannerAdsDialog(
            onSelectPlayer: _selectPlayerNumber, // Pass the callback function
          );
        },
      ).then((value) => setState(
            () {},
    ...
    _playerHighlightAnimCont = AnimationController(duration: Duration(milliseconds: 700), vsync: this);
    _diceHighlightAnimCont = AnimationController(duration: Duration(seconds: 5), vsync: this);
  ...
  void _selectPlayerNumber(int numberOfPlayers) {
    print('Selected $numberOfPlayers players');
    setState(() {
                    ...
                    child: AspectRatio(
                      ...
                      child: Stack(
                        children: <Widget>[
                          SizedBox.expand(
                            child: CustomPaint(
                              painter: BoardPainter(trackCalculationListener: (playerTracks) {
...
List<Widget> _buildPawnWidgets() {
          ...
          child: AnimatedBuilder(
            builder: (_, child) => CustomPaint(
                painter: PlayersPainter(
                    playerCurrentSpot: _playerAnimList[playerIndex][pawnIndex].value, playerColor: playerColor)),
 ...
 _initData() {
    ...
    _safeSpots = [playerTrack[1], playerTrack[9], playerTrack[14], playerTrack[22], playerTrack[27], playerTrack[35], playerTrack[40], playerTrack[48]];
  }
    ...
    if (!_diceHighlightAnimCont.isAnimating) if (_stepCounter == 0) {
      for (int pawnIndex = 0; pawnIndex < _pawnCurrentStepInfo[_currentTurn].length; pawnIndex++)
        if (_pawnCurrentStepInfo[_currentTurn][pawnIndex].value.contains(clickOffset)) {
          ...
          } else if (clickedPawnIndex + _diceOutput > _maxTrackIndex) break; //disallow pawn selection because dice number is more than step left
  ...
  _checkDiceResultValidity() {
    ...
    for (var stepInfo in _pawnCurrentStepInfo[_currentTurn]) {
      if (_diceOutput == 6) {
        if (_straightSixesCounter == 3) //change turn in case of 3 straight sixes
          break;
        else if (stepInfo.key + _diceOutput > _maxTrackIndex) //ignore pawn if it can't move 6 steps
          continue;
  ...
  _movePawn({bool considerCurrentStep = false}) {
    ...
    if (_collisionDetails.isReverse!) {
      ...
      currentStepIndex = min(
          _pawnCurrentStepInfo[playerIndex][pawnIndex].key + (considerCurrentStep ? 0 : 1), //condition to avoid incrementing key for initial step
          _maxTrackIndex);
    }
    ...
    if (_collisionDetails.isReverse!) {
      if (currentStepIndex > 0) {
        ...
        _playerAnimList[_collisionDetails.targetPlayerIndex!][_collisionDetails.pawnIndex!] = Tween(begin: currentStepInfo.value.center, end: _playerTracks[_collisionDetails.targetPlayerIndex!][_collisionDetails.pawnIndex!][currentStepIndex - 1].center).animate(animCont);
      ...
      _playerAnimList[playerIndex][pawnIndex] = Tween(begin: currentStepInfo.value.center, end: _playerTracks[playerIndex][pawnIndex][min(currentStepIndex + 1, _maxTrackIndex)].center).animate(CurvedAnimation(parent: animCont, curve: Interval(0.0, 0.5, curve: Curves.easeOutCubic)));
      animCont.forward(from: 0.0);
    } else {
      if (_checkCollision(currentStepInfo))
        ...
        if (currentStepIndex == _maxTrackIndex) {
          _winnerPawnList[_currentTurn].add(_selectedPawnIndex); //add pawn to [_winnerPawnList]
          ...
          if (_winnerPawnList[_currentTurn].length < 4)
            ...
            _provideFreeTurn = false; //to discard free turn if he completes the game
          }
        }
  ...
  bool _checkCollision(MapEntry<int, Rect> currentStepInfo) {
    ...
    if (currentStepInfo.key < 52) //no need to check if the pawn has entered destination lane
    if (!_safeSpots.any((safeSpot) {
      ...
      for (int playerIndex = 0; playerIndex < _pawnCurrentStepInfo.length; playerIndex++) {
        for (int pawnIndex = 0; pawnIndex < _pawnCurrentStepInfo[playerIndex].length; pawnIndex++) {
          if (playerIndex != _currentTurn || pawnIndex != _selectedPawnIndex) if (_pawnCurrentStepInfo[playerIndex][pawnIndex].value.contains(currentStepCenter)) {
      ...
      if (collisions.isEmpty ||
          ...
          collisions.length > 1) //conditions to no collision and group collisions
        _collisionDetails.isReverse = false;
      else {
...
_changeTurn() {
  int totalPlayers = 4; // Default value for 4 players, change as needed
  ...
  if (_winnerPawnList.where((playerPawns) {
    ...
    if (!_provideFreeTurn) {
      do {
        _currentTurn = (_currentTurn + 1) % totalPlayers;
        if (_winnerPawnList[_currentTurn].length != 4) break; //select player if he is not yet a winner
      } while (true);
      _straightSixesCounter = 0;
    } else if (_diceOutput != 6) _straightSixesCounter = 0; //reset 6s counter if free turn is provided by other means
...
class BannerAdsDialog extends StatelessWidget {
  final Function(int) onSelectPlayer; // Define the callback function parameter

  BannerAdsDialog({required this.onSelectPlayer});
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                _selectPlayerNumber(2);
              },
              child: Text('2 Players'),
            ),
            ElevatedButton(
              onPressed: () {
                _selectPlayerNumber(3);
              },
              child: Text('3 Players'),
            ),
            ElevatedButton(
              onPressed: () {
                _selectPlayerNumber(4);
              },
